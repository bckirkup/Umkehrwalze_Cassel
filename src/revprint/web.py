...
def _project_selection(
...
) -> tuple[list[str], list[str], Path | None, str]:
    ...
    return project_options, volume_options, volume_root, selected_profile
...
def _manifest_diagnostics(latest: Path) -> str:
    diag_html = ""
...
def _latest_html(latest: Path | None) -> str:
    ...
    if latest is not None:
        ...
        diag_html = _manifest_diagnostics(latest)
...
def _index() -> str:
    ...
    project_options, volume_options, volume_root, selected_profile = _project_selection(
        projects, project_store, selected_project, selected_volume, selected_profile
    )
    ...
    latest_html = _latest_html(latest)
...
def create_app() -> Flask:
    app = Flask(__name__)
    app.add_url_rule("/", view_func=_index, methods=["GET"])
    app.add_url_rule("/process", view_func=_process, methods=["POST"])
    app.add_url_rule("/htr", view_func=_htr_editor, methods=["GET"])
    app.add_url_rule("/htr/save", view_func=_htr_save, methods=["POST"])
    app.add_url_rule("/file", view_func=_file, methods=["GET"])
    return app
